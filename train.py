...
from model.models import ModelsFactory
...
class Train:
    ...
    def _train_epoch(self, i_epoch):
        ...
        # except Exception:
        #     print ('fail to load Basedmodel, loading makeup model')
        #     if len(self._opt._gpu_ids) > 1:
        #         self._model = torch.nn.DataParallel(self._model, device_ids=self._opt._gpu_ids)
        #         self._model.cuda()
        #     self._model.train()

        i_train_batch = 0
        for train_batch in tqdm(iter(self._dataset_train)):
